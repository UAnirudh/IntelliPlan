...
class StudyCard(db.Model):
    """Spaced repetition card — one per unique question per user."""
    ...
    familiarity = db.Column(db.Float, default=0.0)  # 0.0 – 1.0
    mastery_level = db.Column(db.String(16), default="not_learned")  # not_learned/learning/mastered
    next_review = db.Column(db.String(32), default="")  # YYYY-MM-DD
    ...
    last_seen = db.Column(db.String(32), default="")
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow)

class StudyAnswer(db.Model):
    """One row per answer attempt for history and analytics."""
    ...
    result = db.Column(db.String(16), default="incorrect")  # correct/partial/incorrect
    score_pct = db.Column(db.Integer, default=0)  # 0-100
    critique = db.Column(db.Text, default="")
    memory_anchor = db.Column(db.Text, default="")
    better_example = db.Column(db.Text, default="")
    confidence = db.Column(db.String(8), default="medium")  # low/medium/high
    points_earned = db.Column(db.Integer, default=0)
    topic = db.Column(db.String(255), default="")
    mode = db.Column(db.String(16), default="casual")  # casual/learn/test/review_mistakes
    time_taken_seconds = db.Column(db.Integer, default=0)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)

class StudyTopic(db.Model):
    """Topic/concept tracking for mastery."""
    ...
    mastery_level = db.Column(db.String(16), default="not_learned")
    total_questions = db.Column(db.Integer, default=0)
    correct_answers = db.Column(db.Integer, default=0)
    last_practiced = db.Column(db.String(32), default="")
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow)
...
AI_CONFIG = {
    ...
    "local_model_url": os.getenv("LOCAL_AI_URL", "http://localhost:5001/ai/local-evaluate"),
    "timeout": 30
}
...
def call_local_fallback(prompt):
    ...
    try:
        ...
        raise Exception(f"Local model unavailable: {str(e)}")

def rule_based_fallback(question, correct_answer, user_answer):
    ...
    if ua == ca:
        return {
            "result": "correct",
            "score_pct": 100,
            "what_was_right": "Your answer matches the correct answer exactly.",
            "what_was_missing": "Nothing - perfect answer!",
            "critique": "Excellent work! You've mastered this concept.",
    ...
    if total > 0:
        ...
        if overlap_pct >= 70:
            result = "correct"
            score = 80 + int(overlap_pct * 0.2)
        elif overlap_pct >= 40:
            result = "partial"
            score = int(overlap_pct)
        else:
            result = "incorrect"
            score = int(overlap_pct * 0.5)
    else:
        result = "incorrect"
        score = 0
    ...
    return {
        "result": result,
        "score_pct": score,
        "what_was_right": f"Your answer contained {overlap} relevant terms." if overlap > 0 else "The answer doesn't match the expected response.",
        "what_was_missing": f"Key concepts missing: {', '.join(ca_words - ua_words)}" if overlap < total else "",
        "critique": "Try to recall the specific terminology and key concepts.",
        "memory_anchor": "Focus on understanding the core concept, not just memorizing words.",
        "better_example": correct_answer
    }

def evaluate_answer_with_ai(question, correct_answer, user_answer, topic=""):
    """
    Multi-tier AI evaluation with fallbacks.
    Returns comprehensive feedback dictionary.
    """
    prompt = f"""You are an expert educational evaluator. Evaluate the student's answer against the correct answer.
...
Evaluate using semantic understanding (not keyword matching). The student may phrase things differently but still be correct.

Respond ONLY with valid JSON in this exact format:
{{
  "result": "correct" | "partial" | "incorrect",
  "score_pct": <integer 0-100>,
  "what_was_right": "<what the student got correct, be specific>",
  "what_was_missing": "<what was missing or wrong, be specific and constructive>",
  "critique": "<2-3 sentence constructive critique combining both>",
  "memory_anchor": "<1-2 sentence intuitive memory trick or analogy to remember this concept>",
  "better_example": "<ideal model answer in 1-3 sentences>"
}}
...
def get_study_owner():
    """Return (user_id, guest_id) for the current request."""
    if current_user.is_authenticated:
...
def q_hash(question_text):
    """Create a hash for a question to identify it uniquely."""
    return hashlib.sha256(question_text.strip().lower().encode()).hexdigest()[:32]

def get_or_create_card(uid, gid, q_hash_val, question_text, answer_text, topic=""):
    """Get existing study card or create a new one."""
    ...
    if not card:
        card = StudyCard(
            user_id=uid, guest_session_id=gid,
            question_hash=q_hash_val, question_text=question_text,
            answer_text=answer_text, topic=topic
        )
        db.session.add(card)
        db.session.flush()
    return card
...
def update_topic_mastery(uid, gid, topic_name, was_correct):
    ...
    if topic.total_questions > 0:
        ...
        elif accuracy >= 0.5 or topic.total_questions >= 3:
            topic.mastery_level = "learning"
        else:
            topic.mastery_level = "not_learned"
    ...
    try:
        ...
        return flask.jsonify({
            ...
            "points": {
                ...
                "streak_freeze": p.streak_freeze_count
            },
            "cards": {
                "total": total_cards,
                "mastered": mastered,
...
@app.route("/api/study/cards/due", methods=["GET"])
def get_due_cards():
...
@app.route("/api/study/cards/<int:card_id>", methods=["GET"])
def get_study_card(card_id):
    ...
    return flask.jsonify({
        ...
        "card": {
            ...
            "partial_count": card.partial_count,
            "incorrect_count": card.incorrect_count
        }
    })

@app.route("/api/study/cards/create", methods=["POST"])
...
@app.route("/api/study/evaluate", methods=["POST"])
def evaluate_study_answer():
    """
    Main evaluation endpoint.
    Receives: question, answer, card_id (optional), confidence, mode, time_taken
    Returns: evaluation, points, updated card state
    """
    uid, gid = get_study_owner()
    data = request.json or {}
    ...
    try:
        ...
        answer_record = StudyAnswer(
            ...
            memory_anchor=evaluation.get("memory_anchor", ""),
            better_example=evaluation.get("better_example", ""),
        ...
        return flask.jsonify({
            ...
            "evaluation": {
                "result": result,
                "score_pct": score_pct,
                "what_was_right": evaluation.get("what_was_right", ""),
    ...
    try:
        ...
        return flask.jsonify({
            ...
            "topics": [{
                ...
                "accuracy_pct": round(t.correct_answers / t.total_questions * 100) if t.total_questions > 0 else 0,
...
def get_study_history():
    """Get answer history for analytics."""
    uid, gid = get_study_owner()
    ...
    try:
        ...
        return flask.jsonify({
            ...
            "answers": [{
                "id": a.id,
                "question": a.question_text[:100] + "..." if len(a.question_text) > 100 else a.question_text,
                "user_answer": a.user_answer[:100] + "..." if len(a.user_answer) > 100 else a.user_answer,
                "correct_answer": a.correct_answer[:100] + "..." if len(a.correct_answer) > 100 else a.correct_answer,
                "result": a.result,
    ...
    try:
        ...
        if not answers:
            return flask.jsonify({
                ...
                "by_result": {"correct": 0, "partial": 0, "incorrect": 0},
        ...
        return flask.jsonify({
            ...
            "accuracy_pct": round(correct / len(answers) * 100) if answers else 0,
...
@app.route("/api/study/mistakes", methods=["GET"])
def get_study_mistakes():
...
@app.route("/api/study/test", methods=["POST"])
...
@app.route("/api/study/flashcard/<int:card_id>", methods=["GET"])
...
@app.route("/api/study/flashcard/mark", methods=["POST"])
def mark_flashcard():
